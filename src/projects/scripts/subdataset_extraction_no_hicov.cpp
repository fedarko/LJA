...
int main(int argc, char **argv) {
    CLParser parser({"unique=none", "output-dir=",
                     "threads=16", "k-mer-size=", "window=2000", "base=239", "hicov=500", "debug",
    ...
    for (AlignedRead &read : readStorage) {
        ...
        dbg::Path path = cPath.getPath();
        for (dbg::Edge *edge : path) {
            if (edge -> getCoverage() >= hicov) {
                // Write out the read to a file
                badreados << ">" << read.id << "\n" << aln.Seq() << "\n";
